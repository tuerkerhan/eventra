<script lang="ts">
	...
	function toDisplay(amount: number, from: 'TRY' | 'USD' | 'EUR' = 'TRY'): number {
		return fromTRY(toTRY(amount, from));
	}

	const currSymbol = $derived(displayCurrency === 'TRY' ? '₺' : displayCurrency === 'USD' ? '$' : '€');
	function fmt(v: number, dec = 0) { return `${currSymbol}${v.toLocaleString('tr-TR', { maximumFractionDigits: dec, minimumFractionDigits: dec })}`; }
	function fmtTL(v: number) { return `₺${Math.round(v).toLocaleString('tr-TR')}`; }
	...
	function fmtRate(v: number) { return v.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
	function fmtAmt(e: ExpenseApi) {
		const s = e.currency === 'TRY' ? '₺' : e.currency === 'USD' ? '$' : '€';
		return `${s}${e.amount.toLocaleString('tr-TR', { maximumFractionDigits: 2 })}`;
	}
	...
	function daysLeft(s: string) {
		return Math.ceil((new Date(s + 'T00:00:00').getTime() - today.getTime()) / 86400000);
	}
	...
	const chartMax = $derived.by(() => {
		const raw = Math.max(...chartMonths.flatMap(m => [m.income, m.expense]), 1);
		const mag = Math.pow(10, Math.floor(Math.log10(raw)));
		return Math.ceil(raw / mag) * mag;
	});

	const CX0 = 58, CY0 = 12, CY1 = 188;
	const CX1 = $derived(Math.max(580, chartPeriod * 52));
	const PW = $derived(CX1 - CX0);
	const PH = CY1 - CY0;
	const GW = $derived(PW / chartPeriod);
	const BW = $derived(Math.max(6, GW * 0.32));
	...
	const yTicks = $derived.by(() =>
		Array.from({ length: 5 }, (_, i) => ({ y: CY1 - (i / 4) * PH, label: fmtShort(chartMax * i / 4) }))
	);

	// Donut
	const R = 68, SW = 30, CIRC = 2 * Math.PI * R;
	...
	const recurringGroups = $derived.by(() => {
		...
		return [...map.values()].map(items => {
			...
			return {
				...
				recurrence: sorted[0].recurrence,
				currency: sorted[0].currency,
				items: sorted,
				paidCount: sorted.filter(i => i.is_paid).length,
				total: sorted.length,
				latestAmount: sorted[sorted.length - 1].amount,
				nextDue: sorted.find(i => !i.is_paid)?.due_date ?? null
			};
	...
	// ─── Gider durum yardımcısı ───────────────────────────────────────────────
	const recLbl: Record<string, string> = {
		once: 'Tek', weekly: 'Haftalık', monthly: 'Aylık', yearly: 'Yıllık', custom: 'Özel'
	};

	function expStatus(e: ExpenseApi) {
		if (e.is_paid) return { label: 'Ödendi', color: '#16a34a', dot: 'green' };
		if (e.amount_type === 'variable' && !e.is_approved)
			return { label: 'Tutar Bekleniyor', color: '#8b5cf6', dot: 'purple' };
		const d = daysLeft(e.due_date);
		if (d < 0) return { label: 'Vadesi Geçti', color: '#dc2626', dot: 'red' };
		if (d <= 3) return { label: `${d}g kaldı`, color: '#f59e0b', dot: 'orange' };
		if (d <= 7) return { label: `${d}g kaldı`, color: '#f97316', dot: 'amber' };
		return { label: `${d}g kaldı`, color: '#64748b', dot: 'gray' };
	}

	// ─── Eylemler ────────────────────────────────────────────────────────────
	function changeSign(v: number | null) {
		if (v === null) return '';
		return v >= 0 ? `+%${Math.abs(v).toFixed(1)}` : `-%${Math.abs(v).toFixed(1)}`;
	}
	function changeColor(v: number | null, inv = false) {
		if (v === null) return 'var(--muted)';
		return (inv ? v < 0 : v >= 0) ? '#16a34a' : '#dc2626';
	}
	...
	function openAmountModal(e: ExpenseApi) {
		amountTarget = e;
		finalAmount = e.amount || 0;
	}

	async function confirmAmount() {
		if (!amountTarget || finalAmount <= 0) return;
		await api.patch(`/expenses/${amountTarget.id}`, { amount: finalAmount, is_approved: true });
		amountTarget = null;
		await reloadExpenses();
	}
	...
	async function saveExpense(ev: Event) {
		...
		try {
			await api.post('/expenses', {
				title: form.title, amount: form.amount,
				amount_type: form.amount_type, currency: form.currency,
				recurrence: form.recurrence,
				custom_period_days: form.recurrence === 'custom' ? form.custom_period_days : null,
				due_date: form.due_date, event_id: form.event_id || null,
				include_kdv: form.include_kdv, note: form.note
			});
			await reloadExpenses();
			showForm = false;
			form = { title: '', amount: 0, amount_type: 'fixed', currency: 'TRY', recurrence: 'once', custom_period_days: 30, due_date: '', event_id: '', include_kdv: false, note: '' };
		} finally {
			saving = false;
		}
	}
...
<section class="db-shell">
	...
	<!-- ─── ALT SEKMELER ──────────────────────────────────────────────────── -->
	<nav class="sub-nav">
		<button class="tab-btn" class:active={tab === 'ozet'} onclick={() => (tab = 'ozet')}>
			Finansal Özet
		</button>
		<button class="tab-btn" class:active={tab === 'odemeler'} onclick={() => (tab = 'odemeler')}>
			Ödemeler
			{#if needsAmount.length > 0 || overdue.length > 0}
				<span class="tab-badge">{needsAmount.length + overdue.length}</span>
			{/if}
		</button>
		<button class="tab-btn" class:active={tab === 'araclar'} onclick={() => (tab = 'araclar')}>
			Araçlar
		</button>
	</nav>

	<!-- ══════════════════════════════════════════════════════════════════════ -->
	<!-- FİNANSAL ÖZET SEKMESİ                                                  -->
	<!-- ══════════════════════════════════════════════════════════════════════ -->
	{#if tab === 'ozet'}
		...
		<div class="kpi-grid">
			...
			<div class="kpi-card {needsAmount.length > 0 ? 'warn' : ''}">
				<span>Tutar Bekleniyor</span>
				<strong>{needsAmount.length}</strong>
				<small class="muted">{needsAmount.length > 0 ? 'Değişken gider' : 'Temiz'}</small>
			</div>
			<div class="kpi-card">
		...
		<div class="charts-row">
			<div class="panel chart-panel">
				<div class="panel-head">
					<div><h2>Gelir & Gider Grafiği</h2><p>{displayCurrency} cinsinden</p></div>
					<div class="chart-controls">
						<div class="period-toggle">
							{#each [3, 6, 12] as p}
								<button class:active={chartPeriod === p} onclick={() => (chartPeriod = p)} type="button">{p}A</button>
							{/each}
						</div>
						<div class="legend">
				...
				<svg viewBox="0 0 {CX1 + 20} 220" class="bar-svg" aria-label="Aylık gelir gider grafiği">
					{#each yTicks as tick}
						<line x1={CX0} y1={tick.y} x2={CX1 + 20} y2={tick.y} stroke="var(--line)" stroke-width="1" />
					...
					{/each}
					<line x1={CX0} y1={CY1} x2={CX1 + 20} y2={CY1} stroke="var(--line)" stroke-width="1.5" />
					<line x1={CX0} y1={CY0} x2={CX0} y2={CY1} stroke="var(--line)" stroke-width="1.5" />
				</svg>
			</div>

			<div class="panel donut-panel">
				<div class="panel-head"><div><h2>Bu Ay Tahsilat</h2><p>Gelir dağılımı</p></div></div>
						...
						{#if donutTotal > 0}
							...
							{#if partialLen > 0}
								<circle cx="100" cy="100" r={R} fill="none" stroke="#2563eb" stroke-width={SW}
									stroke-dasharray="{partialLen} {CIRC}" stroke-dashoffset={-paidLen}
									transform="rotate(-90 100 100)" />
							{/if}
							{#if unpaidLen > 0}
								<circle cx="100" cy="100" r={R} fill="none" stroke="#dc2626" stroke-width={SW}
									stroke-dasharray="{unpaidLen} {CIRC}" stroke-dashoffset={-(paidLen + partialLen)}
									transform="rotate(-90 100 100)" />
							{/if}
						{/if}
	...
	{:else if tab === 'odemeler'}
		...
		<div class="kpi-grid">
			...
			<div class="kpi-card {dueSoon.length > 0 ? 'warn' : ''}">
				...
				<small class="muted">7 gün içinde</small>
			</div>
			<div class="kpi-card {needsAmount.length > 0 ? 'warn' : ''}">
				<span>Tutar Bekleniyor</span>
				<strong>{needsAmount.length}</strong>
				<small class="muted">Değişken gider</small>
			</div>
			<div class="kpi-card">
				<span>Toplam Ödenmemiş</span>
				<strong>₺{unpaidTotal.toLocaleString('tr-TR', {maximumFractionDigits:0})}</strong>
				<small class="muted">{unpaidExpenses.length} kalem</small>
			</div>
		</div>
		...
		{#if needsAmount.length > 0}
			<div class="panel">
				<p class="panel-sub">Tutar Bekleniyor <span class="badge purple">{needsAmount.length}</span></p>
				<div class="exp-list">
					{#each needsAmount as e}
						<div class="exp-row">
							<div class="edot purple"></div>
							<div class="exp-main">
								<strong>{e.title}</strong>
								<div class="tags">
									<span class="badge">{recLbl[e.recurrence]}</span>
									{#if e.event_title}<span class="badge blue">{e.event_title}</span>{/if}
									<span class="badge purple">Değişken</span>
								</div>
							</div>
							<div class="exp-amt"><strong class="dim">≈ {fmtAmt(e)}</strong><small class="muted">tahmini</small></div>
							<div class="exp-date"><span>{fmtDate(e.due_date)}</span></div>
							<div class="exp-actions">
								<button class="act-btn purple" onclick={() => openAmountModal(e)}>✎ Tutarı Gir</button>
								<button class="act-btn red" onclick={() => deleteExpense(e)}>✕</button>
							</div>
						</div>
					{/each}
				</div>
		...
		{#if overdue.length > 0}
			<div class="panel">
				<p class="panel-sub">Vadesi Geçmiş <span class="badge red">{overdue.length}</span></p>
				<div class="exp-list">
					{#each overdue as e}
						{@const st = expStatus(e)}
						<div class="exp-row">
							<div class="edot {st.dot}"></div>
							<div class="exp-main">
								<strong>{e.title}</strong>
								<div class="tags">
									<span class="badge">{recLbl[e.recurrence]}</span>
							...
							<div class="exp-amt"><strong>{fmtAmt(e)}</strong></div>
							<div class="exp-date">
								<span>{fmtDate(e.due_date)}</span>
								<span style="color:{st.color};font-weight:800">{st.label}</span>
							</div>
							<div class="exp-actions">
								<button class="act-btn green" onclick={() => markPaid(e)}>✓ Ödendi</button>
								<button class="act-btn red" onclick={() => deleteExpense(e)}>✕</button>
							</div>
						</div>
					{/each}
		...
		<div class="panel">
			<div class="filter-bar">
				{#each [['all','Tümü'],['unpaid','Ödenmemiş'],['paid','Ödendi'],['needsAmount','Tutar Bekleyen']] as [val, lbl]}
					<button class="filter-btn" class:active={listFilter === val}
						onclick={() => (listFilter = val as typeof listFilter)}>{lbl}</button>
				{/each}
				<span class="filter-count muted">{filteredList.length} kayıt</span>
			</div>

			{#if filteredList.length > 0}
				<div class="exp-list" style="margin-top:0.85rem">
					{#each filteredList as e}
						{@const st = expStatus(e)}
						<div class="exp-row" class:paid-row={e.is_paid}>
							<div class="edot {st.dot}"></div>
							<div class="exp-main">
								<strong>{e.title}</strong>
								<div class="tags">
									<span class="badge">{recLbl[e.recurrence]}</span>
									{#if e.amount_type === 'variable'}<span class="badge purple">Değişken</span>{/if}
									{#if e.event_title}<span class="badge blue">{e.event_title}</span>{/if}
									{#if e.include_kdv}<span class="badge gray">KDV dahil</span>{/if}
								</div>
							</div>
							<div class="exp-amt">
								<strong>{fmtAmt(e)}</strong>
								{#if e.currency !== 'TRY'}<small class="muted">{e.currency}</small>{/if}
							</div>
							<div class="exp-date">
								<span>{fmtDate(e.due_date)}</span>
								<span style="color:{st.color};font-weight:800">{st.label}</span>
			...
			{:else}
				<p class="empty-msg muted">Bu filtreyle kayıt yok.</p>
			{/if}
		</div>
			...
			<div class="rec-grid">
				{#each recurringGroups as g}
					<div class="panel rec-card">
						<div class="rec-head">
							<div>
								<strong class="rec-title">{g.title}</strong>
								<div class="tags" style="margin-top:0.3rem">
									<span class="badge">{recLbl[g.recurrence]}</span>
									<span class="badge">{g.currency}</span>
								</div>
							</div>
							<div class="rec-stat">
								<span class="muted">Son tutar</span>
								<strong>{g.currency === 'TRY' ? '₺' : g.currency === 'USD' ? '$' : '€'}{g.latestAmount.toLocaleString('tr-TR', {maximumFractionDigits:2})}</strong>
							</div>
						</div>
						<div class="rec-progress">
							<div class="rec-bar-wrap"><div class="rec-bar" style="width:{g.total > 0 ? Math.round(g.paidCount/g.total*100) : 0}%"></div></div>
							<span class="muted">{g.paidCount}/{g.total} ödendi</span>
						</div>
						{#if g.nextDue}
							<p class="next-due muted">Sonraki: <strong style="color:var(--text)">{fmtDate(g.nextDue)}</strong></p>
						{:else}
							<p class="next-due" style="color:#16a34a;font-size:0.82rem">Tümü ödendi ✓</p>
						{/if}
					</div>
				{/each}
			</div>
...
{#if amountTarget}
	<div class="modal-back" role="dialog" aria-modal="true">
		<div class="modal" onclick={(e) => e.stopPropagation()} role="document">
			...
			<p class="modal-desc">
				<strong>"{amountTarget.title}"</strong> için gerçekleşen nihai tutarı girin.
				Bu işlem gideri onaylayarak ödeme takibine alacak.
			</p>
			...
			<div class="modal-actions">
				<button class="primary-btn" type="button" onclick={confirmAmount} disabled={finalAmount <= 0}>Onayla ve Kaydet</button>
				<button class="cancel-btn" type="button" onclick={() => (amountTarget = null)}>İptal</button>
			</div>
		</div>
	</div>
{/if}
...
{#if showForm}
	<div class="modal-back" role="dialog" aria-modal="true">
		<div class="modal" role="document">
			<div class="modal-head">
				<h2>Yeni Gider</h2>
				<button class="close-btn" type="button" onclick={() => (showForm = false)}>✕</button>
			</div>
			<form onsubmit={saveExpense} class="expense-form">
				<label><span>Başlık *</span><input required bind:value={form.title} placeholder="Elektrik faturası" /></label>

				<div class="form-row">
					<label style="flex:1"><span>Tutar *</span>
						<input type="number" required min="0" step="0.01" bind:value={form.amount} placeholder="0" />
					</label>
				...
				<div class="form-group">
					...
					<div class="btn-group">
						<button type="button" class:active={form.amount_type === 'fixed'} onclick={() => (form.amount_type = 'fixed')}>Sabit</button>
						<button type="button" class:active={form.amount_type === 'variable'} onclick={() => (form.amount_type = 'variable')}>Değişken</button>
					</div>
					{#if form.amount_type === 'variable'}
						<p class="info-note">Tahmini tutar girilebilir. Nihai tutar listedeki "Tutarı Gir" ile belirlenecek.</p>
					{/if}
				</div>
				...
				<div class="form-group">
					...
					<div class="btn-group wrap">
						{#each [['once','Tek seferlik'],['weekly','Haftalık'],['monthly','Aylık'],['yearly','Yıllık'],['custom','Özel']] as [val, lbl]}
							<button type="button" class:active={form.recurrence === val}
								onclick={() => (form.recurrence = val as typeof form.recurrence)}>{lbl}</button>
						{/each}
					</div>
					{#if form.recurrence !== 'once'}
						<p class="info-note">Başlangıç tarihi geçmişteyse aradaki tüm dönemler otomatik oluşturulur.</p>
					{/if}
					{#if form.recurrence === 'custom'}
						<label><span>Periyot (gün)</span><input type="number" min="1" bind:value={form.custom_period_days} /></label>
					{/if}
				</div>

				<label>
					<span>{form.recurrence === 'once' ? 'Vade Tarihi *' : 'Başlangıç Tarihi *'}</span>
					<input type="date" required bind:value={form.due_date} />
				</label>

				<label>
					<span>Bağlı Etkinlik (opsiyonel)</span>
					<select bind:value={form.event_id}>
				...
				<label class="checkbox-label">
					<input type="checkbox" bind:checked={form.include_kdv} />
					<span>Tutar KDV dahil</span>
				</label>

				<label><span>Not (opsiyonel)</span><textarea rows="2" bind:value={form.note} placeholder="Açıklama…"></textarea></label>

				<div class="modal-actions">
					<button class="primary-btn" type="submit" disabled={saving}>{saving ? 'Kaydediliyor…' : 'Kaydet'}</button>
					<button class="cancel-btn" type="button" onclick={() => (showForm = false)}>İptal</button>
				</div>
			</form>
		</div>
	</div>
...
<style>
	...
	/* Alt sekmeler */
	.sub-nav { display: flex; gap: 0.25rem; border-bottom: 2px solid var(--line); padding-bottom: 0; }
	.tab-btn {
		display: inline-flex; align-items: center; gap: 0.45rem;
		padding: 0.6rem 1.25rem; border: none; background: none;
		color: var(--muted); font-weight: 800; font-size: 0.9rem; cursor: pointer;
		border-bottom: 2px solid transparent; margin-bottom: -2px;
		border-radius: 6px 6px 0 0; transition: color 0.15s, border-color 0.15s, background 0.15s;
	}
	.tab-btn:hover { color: var(--text); background: var(--muted-surface); }
	.tab-btn.active { color: var(--accent); border-bottom-color: var(--accent); background: var(--accent-soft); }
	.tab-badge { background: #dc2626; color: #fff; font-size: 0.68rem; font-weight: 900; padding: 0.1rem 0.4rem; border-radius: 99px; }
	...
	.kpi-card.warn { border-color: rgba(245,158,11,0.3); background: rgba(245,158,11,0.04); }
	...
	.panel-sub { color: var(--muted); font-size: 0.75rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 0.85rem; display: flex; align-items: center; gap: 0.5rem; }
	...
	.chart-controls { display: flex; flex-direction: column; align-items: flex-end; gap: 0.5rem; }
	.period-toggle { display: flex; border: 1px solid var(--line); border-radius: 8px; overflow: hidden; }
	.period-toggle button { padding: 0.35rem 0.7rem; font-weight: 800; font-size: 0.78rem; background: var(--surface-strong); color: var(--muted); border: none; cursor: pointer; transition: all 0.15s; }
	.period-toggle button.active { background: var(--accent); color: #fff; }
	.legend { display: flex; gap: 0.85rem; align-items: center; }
	...
	.exp-row { display: grid; grid-template-columns: 10px 1fr auto auto auto; align-items: center; gap: 0.85rem; padding: 0.8rem; background: var(--surface-strong); border: 1px solid var(--line); border-radius: 8px; transition: border-color 0.15s; }
	.exp-row:hover { border-color: color-mix(in srgb, var(--accent) 30%, transparent); }
	.exp-row.paid-row { opacity: 0.6; }
	.edot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
	.edot.green { background: #16a34a; }
	.edot.red { background: #dc2626; }
	.edot.orange { background: #f97316; }
	.edot.amber { background: #f59e0b; }
	.edot.purple { background: #8b5cf6; }
	.edot.gray { background: #64748b; }
	...
	.exp-amt { text-align: right; min-width: 90px; }
	.exp-amt strong { font-size: 0.96rem; }
	.exp-amt small { display: block; }
	.dim { color: var(--muted) !important; }
	.exp-date { text-align: right; min-width: 110px; font-size: 0.78rem; display: flex; flex-direction: column; gap: 0.15rem; }
	.exp-date span { color: var(--muted); }
	.exp-actions { display: flex; gap: 0.4rem; }
	.act-btn { border: none; border-radius: 7px; padding: 0.35rem 0.65rem; font-weight: 800; font-size: 0.76rem; cursor: pointer; white-space: nowrap; }
	.act-btn.green { background: rgba(22,163,74,0.12); color: #16a34a; }
	.act-btn.red { background: rgba(220,38,38,0.1); color: #dc2626; }
	.act-btn.purple { background: rgba(139,92,246,0.12); color: #8b5cf6; }

	/* Filter bar */
	.filter-bar { display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap; }
	.filter-btn { padding: 0.4rem 0.85rem; border: 1px solid var(--line); border-radius: 7px; background: var(--surface-strong); color: var(--muted); font-weight: 800; font-size: 0.82rem; cursor: pointer; transition: all 0.15s; }
	.filter-btn.active { border-color: var(--accent); background: var(--accent-soft); color: var(--text); }
	.filter-count { margin-left: auto; font-size: 0.82rem; }
	.empty-msg { padding: 1.5rem; text-align: center; }
	...
	/* Recurring */
	.rec-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 0.85rem; }
	.rec-card { display: flex; flex-direction: column; gap: 0.7rem; }
	.rec-head { display: flex; justify-content: space-between; align-items: flex-start; gap: 0.75rem; }
	.rec-title { font-size: 0.95rem; }
	.rec-stat { text-align: right; display: flex; flex-direction: column; gap: 0.2rem; }
	.rec-stat span { font-size: 0.75rem; color: var(--muted); font-weight: 800; }
	.rec-stat strong { font-size: 1rem; }
	.rec-progress { display: flex; flex-direction: column; gap: 0.35rem; }
	.rec-bar-wrap { height: 5px; background: var(--muted-surface); border-radius: 99px; overflow: hidden; }
	.rec-bar { height: 100%; background: #16a34a; border-radius: 99px; }
	.next-due { font-size: 0.8rem; }
	...
	.modal { background: var(--surface); border: 1px solid var(--line); border-radius: 14px; width: 100%; max-width: 500px; max-height: 92vh; overflow-y: auto; padding: 1.5rem; box-shadow: 0 40px 80px rgba(0,0,0,0.4); }
	...
	.modal-desc { color: var(--muted); font-size: 0.9rem; margin-bottom: 1rem; line-height: 1.5; }
	.hint { font-size: 0.8rem; margin-top: 0.5rem; }
	.amount-wrap { display: flex; align-items: center; border: 1px solid var(--line); border-radius: 8px; overflow: hidden; background: var(--surface-strong); }
	.curr-prefix { padding: 0 0.75rem; font-size: 1.1rem; font-weight: 900; color: var(--accent); border-right: 1px solid var(--line); background: var(--muted-surface); height: 44px; display: flex; align-items: center; }
	.amount-inp { border: none !important; border-radius: 0 !important; font-size: 1.1rem; font-weight: 800; padding: 0.6rem 0.85rem; background: transparent; color: var(--text); width: 100%; }
	.amount-inp:focus { outline: none; }
	.modal-actions { display: flex; gap: 0.65rem; padding-top: 0.75rem; }
	...
	.info-note { font-size: 0.78rem; color: #8b5cf6; font-weight: 700; margin: 0; }
	...
	@media (max-width: 640px) {
		...
		.exp-row { grid-template-columns: 10px 1fr; }
		.exp-amt, .exp-date, .exp-actions { grid-column: 2; }
	}
</style>
